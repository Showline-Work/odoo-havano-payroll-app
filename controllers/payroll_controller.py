...
try:
    ...
    # Test if it actually works by accessing a property
    WeasyHTML
    WEASYPRINT_AVAILABLE = True
    _logger.info("✅ WeasyPrint loaded - PDF reports will be fast (15-20x)")
except (ImportError, OSError, SystemError, Exception):
    WEASYPRINT_AVAILABLE = False
    # Silent fallback - WeasyPrint requires GTK3 on Windows
...
class ZimbabwePayrollController(http.Controller):
    ...
    # ============================================================
    # CORE PDF RENDERING ENGINE (WeasyPrint → wkhtmltopdf fallback)
    # ============================================================
    @staticmethod
    def _render_pdf(report_name, docids, data=None):
        """
        Fast PDF rendering using WeasyPrint.
        Falls back to Odoo's default wkhtmltopdf if WeasyPrint is not installed.
        """
        # Step 1: Get HTML from QWeb template (same for both engines)
        html_content, _ = request.env['ir.actions.report']._render_qweb_html(
            report_name, docids, data=data
        )
        
        if WEASYPRINT_AVAILABLE:
            # Step 2a: Convert to PDF with WeasyPrint (fast)
            try:
                return WeasyHTML(string=html_content).write_pdf()
            except Exception as e:
                _logger.warning(f"WeasyPrint failed: {e}. Falling back to wkhtmltopdf.")
                # Fall through to wkhtmltopdf
        
        # Step 2b: Convert to PDF with wkhtmltopdf (fallback)
        return request.env['ir.actions.report']._run_wkhtmltopdf(html_content)
    ...
    def salary_breakdown_pdf(self, pay_run_id, **kwargs):
        ...
        pdf_content = self._render_pdf(report.report_name, [pay_run.id], data=data)
    ...
    def export_report(self, report_type, pay_run_id, format='pdf', **kwargs):
        ...
        routes = {
            'salary_breakdown': (self._export_salary_breakdown_excel, self.salary_breakdown_pdf),
            'salary_summary': (self._export_salary_summary_excel, self.salary_summary_pdf),
            'detailed_breakdown': (self._export_detailed_breakdown_excel, self.detailed_breakdown_pdf),
            'summary_breakdown': (self._export_summary_breakdown_excel, self.summary_breakdown_pdf),
            'nssa': (self._export_nssa_excel, self.nssa_report_pdf),
            'nssa_p4': (self._export_nssa_p4_excel, self.nssa_p4_report_pdf),
            'zimra_itf': (self._export_zimra_itf_excel, self.zimra_itf_report_pdf),
            'nec': (self._export_nec_excel, self.nec_report_pdf),
            'zimdef': (self._export_zimdef_excel, self.zimdef_report_pdf),
        }

        if report_type in routes:
            excel_func, pdf_func = routes[report_type]
            return excel_func(pay_run) if export_format == 'excel' else pdf_func(pay_run_id)

        return request.not_found()

    # ============================================================
    # EXCEL EXPORTS (unchanged - xlsxwriter is fast enough)
    # ============================================================
